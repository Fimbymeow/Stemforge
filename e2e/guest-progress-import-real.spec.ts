...
test("confirmed real-auth import is durable, idempotent, local-preserving and retry-safe", async ({ page, seriousBrowserErrors }) => {
  ...
  await expect(page.getByTestId("guest-progress-import")).toContainText("ready to add");
  ...
  await expect(page.getByTestId("guest-progress-import")).toContainText("Browser progress was added");
  ...
  await page.reload();
  await expect(page.getByTestId("guest-progress-import")).toContainText("already been added");
  ...
  await page.reload();
  await expect(page.getByTestId("guest-progress-import")).toContainText("previously added to a different account");
...
test("authenticated account with no local evidence shows a quiet empty state", async ({ page, seriousBrowserErrors }) => {
  ...
  await signIn(page);
  await expect(page.getByTestId("guest-progress-import")).toContainText("No progress is saved in this browser yet");
  expect(seriousBrowserErrors).toEqual([]);
});
