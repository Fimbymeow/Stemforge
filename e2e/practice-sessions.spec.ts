...
test("guest targeted practice starts, uses the canonical question workspace, persists, and summarizes", async ({ page }) => {
  ...
  await expect(page.getByTestId("practice-session-panel")).toContainText("Question 1 of");
  const finishTrigger = page.getByTestId("practice-session-panel").getByRole("button", { name: "Finish session" });
  await finishTrigger.click();
  const finishDialog = page.getByRole("dialog", { name: "Finish this session?" });
  await expect(finishDialog).toHaveAttribute("data-dialog-shell", "true");
  await expect(finishDialog.getByRole("button", { name: "Return to practice" })).toBeFocused();
...
test("question list, reversible Skip and 320px session chrome remain usable without overflow", async ({ page }) => {
  ...
  const pickerTrigger = panel.getByRole("button", { name: /Question 1 of/i });
  await pickerTrigger.click();
  const list = page.getByRole("dialog", { name: "Session questions" });
  await expect(list).toHaveAttribute("data-dialog-shell", "true");
  await expect(list.getByRole("button", { name: "Close question list" })).toBeFocused();
  await page.keyboard.press("Escape");
