...
export function evidenceRecordDigest(
  ...
  kind: "attempt" | "support_event" | "guided_self_assessment" | "achievement_snapshot" | "review_event" | "flashcard_review",
  eventId: string,
) {
  const value = kind === "attempt"
    ...
    : kind === "support_event"
      ...
      : kind === "guided_self_assessment"
        ...
        : kind === "achievement_snapshot"
          ? payload.data.achievementSnapshots.find((item) => item.snapshotId === eventId)
          : kind === "review_event"
            ? payload.data.reviewEvents.find((item) => item.eventId === eventId)
            : payload.data.flashcardReviews.find((item) => item.eventId === eventId);
...
export function pendingProgressSyncEvidence(payload: ProgressPayload, metadata: ProgressSyncMetadata, fingerprint: string): ProgressPayload {
  const account = metadata.accounts[fingerprint] ?? createDefaultProgressSyncAccount();
  const pending = (kind: "attempt" | "support_event" | "guided_self_assessment" | "achievement_snapshot" | "review_event" | "flashcard_review", eventId: string) => {
  ...
  return {
    ...
    data: {
      ...
      reviewEvents: payload.data.reviewEvents.filter((item) => pending("review_event", item.eventId)),
      flashcardReviews: payload.data.flashcardReviews.filter((item) => pending("flashcard_review", item.eventId)),
    },
  };
}
...
function validReference(value: string) {
  return /^(attempt|support_event|guided_self_assessment|achievement_snapshot|review_event|flashcard_review):[A-Za-z0-9][A-Za-z0-9._:-]{0,199}$/.test(value);
}
