exports.up = (pgm) => {
  pgm.sql(`
    CREATE TABLE stemforge_remote.review_events (
      ...
      CONSTRAINT review_events_owner_event_unique UNIQUE (owner_id, event_id)
    );

    CREATE INDEX review_events_owner_receive_idx
      ON stemforge_remote.review_events (owner_id, receive_order);

    CREATE TRIGGER review_events_set_hash
      BEFORE INSERT ON stemforge_remote.review_events
      FOR EACH ROW EXECUTE FUNCTION stemforge_remote.set_evidence_payload_hash();
    CREATE TRIGGER review_events_verify_generation
      BEFORE INSERT ON stemforge_remote.review_events
      FOR EACH ROW EXECUTE FUNCTION stemforge_account_data.verify_evidence_generation();
    CREATE TRIGGER review_events_no_mutation
      BEFORE UPDATE OR DELETE ON stemforge_remote.review_events
      FOR EACH ROW EXECUTE FUNCTION stemforge_remote.reject_evidence_mutation();
    CREATE TRIGGER review_events_no_truncate
      BEFORE TRUNCATE ON stemforge_remote.review_events
      FOR EACH STATEMENT EXECUTE FUNCTION stemforge_remote.reject_evidence_mutation();

    REVOKE UPDATE, DELETE, TRUNCATE ON stemforge_remote.review_events FROM PUBLIC;
    ...
    ALTER TABLE stemforge_remote.evidence_conflicts
      ...
      CHECK (evidence_kind IN (
        'attempt', 'support_event', 'guided_self_assessment', 'achievement_snapshot', 'review_event'
      ));

    ALTER TABLE stemforge_account_data.requests
      ADD COLUMN deleted_review_event_count bigint
      CHECK (deleted_review_event_count IS NULL OR deleted_review_event_count >= 0);
    ...
    RETURNS TABLE (
      ...
      deleted_review_events bigint, deleted_conflicts bigint
    )
    ...
    DECLARE review_events_count bigint;
    ...
    BEGIN
      ...
      DELETE FROM stemforge_remote.review_events WHERE owner_id = request_row.owner_id;
      GET DIAGNOSTICS review_events_count = ROW_COUNT;
      ...
      UPDATE stemforge_account_data.requests SET status = 'completed', generation_after = next_generation,
        ...
        deleted_achievement_snapshot_count = snapshots_count,
        deleted_review_event_count = review_events_count,
        deleted_conflict_count = conflicts_count,
      ...
      RETURN QUERY SELECT next_generation, attempts_count, support_count, self_assessments_count,
        snapshots_count, review_events_count, conflicts_count;
    END;
    $$;
...
exports.down = () => {
  throw new Error("Review evidence is forward-only; destructive automatic rollback is intentionally unavailable.");
};
