...
export function AppSidebar({
  ...
  const visibleNavItems = accountsAvailable
    ...
    : navItems;
  return (
    <aside data-app-sidebar className="fixed inset-y-0 left-0 z-10 flex w-[268px] flex-col border-r border-line bg-paper/90 px-5 py-8 backdrop-blur-xl max-xl:sticky max-xl:top-0 max-xl:h-auto max-xl:w-full max-xl:border-b max-xl:border-r-0 max-xl:px-5 max-xl:py-4">
      ...
      <nav aria-label="Main" className="grid gap-3 max-xl:flex max-xl:gap-1">
        {visibleNavItems.map(([label, Icon, key, shortLabel]) => {
          if (key === "current-path" && workingContextPathId) {
            return <WorkingContextNavigation key={label} pathId={workingContextPathId} />;
          }
          return (
            <Link
              key={label}
