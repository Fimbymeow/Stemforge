  ...
  const filterPanel = (mobile: boolean) => (
    <div className="grid gap-5">
      ...
      <StageMultiFilter
        ...
        allLabel="All stages"
      />
      <details className="group disclosure-motion border-y border-rule">
        <summary className="orthic-secondary-link flex min-h-11 cursor-pointer list-none items-center justify-between font-medium text-sm">More filters<ChevronDown aria-hidden="true" className="orthic-disclosure-chevron size-4 group-open:rotate-180" /></summary>
        <div className="grid gap-4 pb-4 pt-2">
          <label className="grid gap-1 text-sm font-bold">Status
            <select value={rawUrlState.status} onChange={(event) => updateFilters({ status: event.target.value as QuestionBankProgressFilter })} className="min-h-11 min-w-0 rounded border border-rule bg-white px-3">
          ...
          <label className="grid gap-1 text-sm font-bold">Question type
            <select value={effectiveType} onChange={(event) => updateFilters({ type: event.target.value as QuestionBankUrlState["type"] })} className="min-h-11 min-w-0 rounded border border-rule bg-white px-3">
          ...
          {options.hasCalculatorQuestions ? (
            <label className="grid gap-1 text-sm font-bold">Calculator
              <select value={rawUrlState.calc} onChange={(event) => updateFilters({ calc: event.target.value as QuestionBankUrlState["calc"] })} className="min-h-11 min-w-0 rounded border border-rule bg-white px-3">
      ...
      <div className="grid gap-3 border-t border-line pt-3 lg:hidden">
        <label className="grid gap-1 text-sm font-bold">Sort
          <select value={rawUrlState.sort} onChange={(event) => updateFilters({ sort: event.target.value as QuestionBankUrlState["sort"] })} className="min-h-11 min-w-0 rounded border border-rule bg-white px-3">
        ...
        {filteredEligibleIds.length ? <button type="button" onClick={() => setSelected((current) => setQuestionGroupSelection(current, filteredEligibleIds, true))} className="min-h-11 rounded border border-rule bg-white px-3 font-bold text-forge">Select all {filteredEligibleIds.length} filtered questions</button> : null}
      </div>
      <button type="button" onClick={resetAllFilters} className="orthic-transition-fast min-h-11 rounded border border-rule bg-white px-3 font-bold text-forge">Reset filters</button>
    </div>
  );

  return (
    <AppShell demo active="Subjects" workingContextPathId={normalizedFilters.skillPathIds.length === 1 ? normalizedFilters.skillPathIds[0] : null} feedbackPlacement="inline-mobile" contextBar={<><nav className="flex flex-wrap items-center gap-2 text-sm text-secondary" aria-label="Breadcrumb"><Link href={subject.href} className="orthic-secondary-link inline-flex min-h-11 items-center">{subject.subjectName}</Link><ArrowRight aria-hidden="true" className="size-4" /><span className="font-semibold text-navy">Question Bank</span></nav><AppTopbar demo /></>}>
      <div
        className="question-bank-v2 mx-auto grid min-w-0 max-w-[1220px] gap-6"
        style={selected.size ? { paddingBottom: "calc(var(--question-bank-selection-height, 4rem) + var(--fixed-ui-gap))" } : undefined}
      >
        <header>
          <h1 className="m-0 text-[32px] font-bold leading-tight tracking-tight text-navy">Question Bank</h1>
          <p className="mt-2 max-w-3xl text-[15px] leading-relaxed text-secondary">Choose skills and questions to build your own practice session.</p>
          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-sm text-secondary">
            <span>{allEntries.length} questions available</span>
            <span>{options.skillPaths.length} available skill{options.skillPaths.length === 1 ? "" : "s"}</span>
            <span>More {subject.subjectName} content is coming later</span>
          </div>
        </header>

        {scopedSkillPath || reviewDueEligible.length ? <div className="flex flex-wrap items-center gap-2">
          {scopedSkillPath ? <p className="inline-flex flex-wrap items-center gap-2 text-sm font-medium">
            Scoped to {scopedSkillPath.name}
            <button type="button" onClick={() => updateFilters({ skillPathIds: [], stageIds: [] })} className="min-h-11 rounded border border-rule px-2 text-xs font-bold text-forge">Browse all {subject.subjectName}</button>
          </p> : null}
          {reviewDueEligible.length ? <button ref={reviewDueTriggerRef} type="button" onClick={() => setReviewDueConfirmOpen(true)} className="orthic-transition-fast inline-flex min-h-11 items-center gap-2 rounded border border-amber-200 bg-amber-50 px-3 text-sm font-medium text-amber-800">
            {formatNeedsPracticeLabel(reviewDueEligible.length)}
          </button> : null}
        </div> : null}

        <div className="grid min-w-0 gap-6 lg:grid-cols-[220px_minmax(0,1fr)] xl:grid-cols-[240px_minmax(0,1fr)] xl:gap-8">
          <div id="question-bank-filters" className="hidden h-fit border-r border-rule pr-6 lg:block">
            <p className="mb-5 inline-flex items-center gap-2 font-semibold text-navy"><Filter aria-hidden="true" className="size-4" />Filters</p>
            {filterPanel(false)}
          </div>
          ...
          <section className="min-w-0" aria-labelledby="question-results-title">
            <div className="grid gap-3 max-md:gap-2">
              <div className="flex items-center gap-2">
                <label className="relative block min-w-0 flex-1">
                  ...
                  <input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Search questions, skills or stages" className="min-h-12 w-full rounded border border-rule bg-white pl-10 pr-4 max-md:min-h-11" />
                </label>
                <button ref={filterTriggerRef} type="button" onClick={() => setMobileFiltersOpen(true)} className="inline-flex min-h-11 w-fit shrink-0 items-center gap-2 rounded border border-rule bg-white px-3 font-bold lg:hidden">
              ...
              {activeChips.length ? <div className="flex flex-wrap items-center gap-2">
                {activeChips.map((chip) => <button key={`${chip.key}:${chip.value ?? ""}`} type="button" onClick={() => removeChip(chip)} aria-label={`Remove ${chip.label} filter`} className="inline-flex min-h-11 items-center gap-1 rounded border border-rule bg-white px-3 text-xs font-bold">{chip.label}<X className="size-3" /></button>)}
              </div> : null}

              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-rule pb-3">
                <h2 id="question-results-title" className="text-base font-semibold text-navy" aria-live="polite">{questions.length} matching question{questions.length === 1 ? "" : "s"}</h2>
                <div className="flex flex-wrap items-center gap-2 max-md:hidden">
                  {questions.length ? (
                    <button type="button" onClick={() => setSelected((current) => setQuestionGroupSelection(current, filteredEligibleIds, true))} className="orthic-secondary-link min-h-11 rounded border border-rule bg-white px-3 text-sm font-medium text-secondary">
                      Select all {filteredEligibleIds.length} filtered questions
                    </button>
                  ) : null}
                  <label className="text-sm font-bold">Sort <select value={rawUrlState.sort} onChange={(event) => updateFilters({ sort: event.target.value as QuestionBankUrlState["sort"] })} className="ml-1 min-h-11 rounded border border-rule bg-white px-2">
                    <option value="default">Course order</option><option value="recently-practised">Recently practised</option><option value="review-priority">Review priority</option><option value="completion-status">Completion status</option>
                  </select></label>
                </div>
              </div>
            ...
            {pagination.pageCount > 1 ? <nav className="mt-4 flex flex-wrap items-center justify-between gap-3" aria-label="Question result pages">
              ...
              <div className="flex gap-2"><button type="button" disabled={pagination.page === 1} onClick={() => goToPage(pagination.page - 1)} className="min-h-11 rounded border border-rule bg-white px-3 font-bold disabled:opacity-40"><ArrowLeft className="mr-1 inline size-4" />Previous</button><button type="button" disabled={pagination.page === pagination.pageCount} onClick={() => goToPage(pagination.page + 1)} className="min-h-11 rounded border border-rule bg-white px-3 font-bold disabled:opacity-40">Next<ArrowRight className="ml-1 inline size-4" /></button></div>
            </nav> : null}
          </section>
        </div>

        <details className="group disclosure-motion border-t border-rule">
          <summary className="flex min-h-12 cursor-pointer list-none items-center justify-between gap-3 px-5 py-3 font-bold text-muted"><span className="inline-flex items-center gap-2"><Lock className="size-4" />Future {subject.subjectName} coverage</span><ChevronDown className="orthic-disclosure-chevron size-4 group-open:rotate-180" /></summary>
        ...
        >
          ...
          <div className="flex flex-wrap gap-2"><button type="button" onClick={() => setSelected(new Set())} className="min-h-11 px-3 font-bold text-muted">Clear selection</button><button type="button" onClick={() => setReviewOpen(true)} className="min-h-11 rounded border border-rule px-3 font-bold">Review selection</button><button type="button" onClick={startPractice} className="min-h-11 rounded-lg orthic-primary-action bg-navy px-4 font-semibold text-white">Start selected practice</button></div>
        </section> : null}
      </div>
...
function FilterSelect({ label, value, onChange, options, allLabel }: { label: string; value: string; onChange: (value: string) => void; options: readonly { id: string; name: string }[]; allLabel: string }) {
  return <label className="grid gap-1 text-sm font-bold">{label}<select aria-label={label} value={value} onChange={(event) => onChange(event.target.value)} className="min-h-11 min-w-0 rounded border border-rule bg-white px-3"><option value="">{allLabel}</option>{options.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>;
}
...
function StageMultiFilter({ label, values, onClear, onToggle, options, allLabel }: { label: string; values: readonly string[]; onClear: () => void; onToggle: (ids: readonly string[], checked: boolean) => void; options: readonly { name: string; ids: string[] }[]; allLabel: string }) {
  return <fieldset className="border-t border-rule pt-3">
    ...
    <div className="grid gap-1 border-t border-line pt-1">
      {options.map((item) => {
        ...
        return <label key={item.name} className="flex min-h-11 items-center gap-2 text-sm font-semibold">
...
function MobileFilterSheet({ containerRef, closeRef, onClose, children }: { containerRef: React.RefObject<HTMLElement | null>; closeRef: React.RefObject<HTMLButtonElement | null>; onClose: () => void; children: React.ReactNode }) {
  ...
  return <div className="fixed inset-0 z-50 bg-ink/35 lg:hidden" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    <section ref={containerRef} role="dialog" aria-modal="true" aria-labelledby={titleId} data-question-bank-dialog className="orthic-transition-standard !duration-[180ms] fixed inset-x-0 bottom-0 flex max-h-[90dvh] flex-col rounded-t-lg border-t border-line bg-white shadow-2xl">
      <header className="flex items-center justify-between gap-3 border-b border-line p-4">
        ...
        <button ref={closeRef} type="button" onClick={onClose} aria-label="Close filters" className="orthic-transition-fast grid size-11 place-items-center rounded border border-rule"><X className="size-5" /></button>
      </header>
      <div className="min-h-0 flex-1 overflow-y-auto p-4">{children}</div>
      <footer className="flex gap-2 border-t border-line p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
        <button type="button" onClick={onClose} className="min-h-11 w-full rounded-lg orthic-primary-action bg-navy px-3 font-semibold text-white">Apply</button>
      </footer>
    </section>
  </div>;
}
...
function QuestionRow({ entry, selected, expanded, onSelected, onToggleExpand }: { entry: QuestionBankQuestionEntry; selected: boolean; expanded: boolean; onSelected: (checked: boolean) => void; onToggleExpand: () => void }) {
  ...
  // Keep authored math delimiters intact; never truncate through a TeX expression.
  const excerpt = entry.question.questionText.split(/\n\s*\n/)[0];
...
function QuestionBankEmptyState({ hasAnyQuestions, scopedSkillPathName, hasSearch, hasActiveFilters, previouslyIncorrect, activeChips, onClearSearch, onAdjustFilters, onResetFilters, onRemoveChip, subjectName, subjectHref }: {
  ...
  if (!hasAnyQuestions) {
    return <Card className="mt-3 !rounded-lg !border-rule p-5 !shadow-none">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Link href={subjectHref} className="inline-flex min-h-11 items-center rounded border border-rule px-4 font-bold text-forge">Return to subject overview</Link>
        <Link href="/subjects" className="inline-flex min-h-11 items-center rounded border border-rule px-4 font-bold text-forge">Browse another available area</Link>
      </div>
    </Card>;
  }
  if (scopedSkillPathName) {
    return <Card className="mt-3 !rounded-lg !border-rule p-5 !shadow-none">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button type="button" onClick={onResetFilters} className="orthic-transition-fast min-h-11 rounded border border-rule px-4 font-bold text-forge">Browse all {subjectName}</button>
      </div>
    </Card>;
  }
  if (hasSearch) {
    return <Card className="mt-3 !rounded-lg !border-rule p-5 !shadow-none">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button type="button" onClick={onClearSearch} className="orthic-transition-fast min-h-11 rounded border border-rule px-4 font-bold text-forge">Clear search</button>
        <button type="button" onClick={onAdjustFilters} className="orthic-transition-fast min-h-11 rounded border border-rule px-4 font-bold text-forge">Adjust filters</button>
      </div>
    </Card>;
  }
  if (previouslyIncorrect) {
    return <Card className="mt-3 !rounded-lg !border-rule p-5 !shadow-none">
      ...
      <button type="button" onClick={onResetFilters} className="orthic-transition-fast mt-4 min-h-11 rounded border border-rule px-4 font-bold text-forge">Clear filters</button>
    </Card>;
  }
  return <Card className="mt-3 !rounded-lg !border-rule p-5 !shadow-none">
    <h3 className="font-extrabold">No questions match these filters</h3>
    {activeChips.length ? <div className="mt-3 flex flex-wrap gap-2">{activeChips.map((chip) => <button key={`${chip.key}:${chip.value ?? ""}`} type="button" onClick={() => onRemoveChip(chip)} className="inline-flex min-h-11 items-center gap-1 rounded border border-rule bg-white px-3 text-xs font-bold">{chip.label}<X className="size-3" /></button>)}</div> : null}
    {hasActiveFilters ? <button type="button" onClick={onResetFilters} className="orthic-transition-fast mt-4 min-h-11 rounded border border-rule px-4 font-bold text-forge">Reset all filters</button> : null}
  </Card>;
}
...
function ReviewSelection({ entries, onClose, onRemove, onClear, onStart, closeRef }: { entries: QuestionBankQuestionEntry[]; onClose: () => void; onRemove: (id: string) => void; onClear: () => void; onStart: () => void; closeRef: React.RefObject<HTMLButtonElement | null> }) {
  return <div className="fixed inset-0 z-50 grid place-items-center bg-ink/45 p-3" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-labelledby="review-selection-title" data-question-bank-dialog className="orthic-transition-standard !duration-[180ms] max-h-[85vh] w-full max-w-2xl overflow-auto rounded-lg bg-white p-5 shadow-card">
      <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-extrabold uppercase text-forge">Custom practice</p><h2 id="review-selection-title" className="text-2xl font-extrabold">Review selection</h2><p className="mt-1 text-sm text-muted">{entries.length} questions · {entries.reduce((total, entry) => total + entry.question.marks, 0)} marks</p></div><button ref={closeRef} type="button" onClick={onClose} aria-label="Close selection review" className="orthic-transition-fast grid size-11 place-items-center rounded border border-rule"><X className="size-5" /></button></div>
      <ul className="mt-4 divide-y divide-line">{entries.map((entry) => <li key={entry.question.id} className="flex items-center justify-between gap-3 py-3"><div><p className="text-xs font-bold text-muted">{entry.context.skillPath.name} · {entry.context.stage.name}</p><p className="font-bold"><InlineMathContent>{entry.question.title}</InlineMathContent></p></div><button type="button" onClick={() => onRemove(entry.question.id)} aria-label={`Remove ${entry.question.title}`} className="min-h-11 px-2 font-bold text-muted">Remove</button></li>)}</ul>
      <div className="mt-4 flex flex-wrap justify-between gap-2"><button type="button" onClick={onClear} className="orthic-transition-fast min-h-11 px-3 font-bold text-muted">Clear all</button><div className="flex gap-2"><button type="button" onClick={onClose} className="orthic-transition-fast min-h-11 rounded border border-rule px-3 font-bold">Return to filters</button><button type="button" onClick={onStart} disabled={!entries.length} className="min-h-11 rounded-lg orthic-primary-action bg-navy px-4 font-semibold text-white disabled:opacity-40">Start selected practice</button></div></div>
    </section>
  </div>;
}
...
function ReviewDueConfirmation({ containerRef, closeRef, eligibleCount, ineligibleCount, scope, onClose, onStart }: {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-ink/45 p-3" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    <section ref={containerRef} role="dialog" aria-modal="true" aria-labelledby="review-due-title" data-question-bank-dialog className="orthic-transition-standard !duration-[180ms] w-full max-w-lg rounded-lg bg-white p-5 shadow-card">
      <div className="flex items-start justify-between gap-3">
        ...
        <button ref={closeRef} type="button" onClick={onClose} aria-label="Close review confirmation" className="orthic-transition-fast grid size-11 place-items-center rounded border border-rule"><X className="size-5" /></button>
      </div>
      ...
      <div className="mt-5 flex flex-wrap justify-end gap-2">
        <button type="button" onClick={onClose} className="orthic-transition-fast min-h-11 px-3 font-bold text-muted">Cancel</button>
        <button type="button" onClick={onStart} className="min-h-11 rounded-lg orthic-primary-action bg-navy px-4 font-semibold text-white">Start practice again</button>
      </div>
    </section>
  </div>;
}
